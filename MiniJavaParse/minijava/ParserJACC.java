...
class ParserJACC implements Tokens {
    ...
    private int yyr5() { // proc : PROCEDURE ID '(' ')' cmds END
        { yyrv = new Proc(((Token)yysv[yysp-5]).lexeme, ((Bloco)yysv[yysp-2]), ((Token)yysv[yysp-6]).lin); }
        yysv[yysp-=6] = yyrv;
        switch (yyst[yysp-1]) {
            case 0: return 3;
            default: return 21;
    ...
    private int yyr18() { // cmd : ID '(' ')'
        { yyrv = new Chamada(((Token)yysv[yysp-3]).lexeme, ((Token)yysv[yysp-3]).lin); }
        yysv[yysp-=3] = yyrv;
        return yypcmd();
    }
    ...
    private int yyr9() { // var : /* empty */
        { yyrv = new Bloco(new java.util.ArrayList<String>(), ((Token)token).lin); }
        yysv[yysp-=0] = yyrv;
        return 5;
    }
